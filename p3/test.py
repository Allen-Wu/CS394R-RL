...
def test_tile():
    ...
    print(Vs)
    assert np.allclose(Vs,correct_values,1e-2,3), '{} != {}, but it might due to stochasticity'.format(correct_values, Vs)
...
def test_nn():
    ...
    Vs = [V(s) for s in testing_states]
    print(Vs)
    assert np.allclose(Vs,correct_values,0.20,5.), '{} != {}, but it might due to stochasticity'.format(correct_values, Vs)
